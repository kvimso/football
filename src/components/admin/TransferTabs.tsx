...
export function TransferTabs({ incoming, outgoing, pendingIncoming, pendingOutgoing }: TransferTabsProps) {
  const { t } = useLang()
    ...
    <div>
      ...
      <div className="inline-flex rounded-xl border border-white/[0.06] bg-white/[0.02] p-1">
        ...
        >
          ...
          {pendingIncoming > 0 && (
            <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-yellow-500/15 px-1 text-[10px] font-bold text-yellow-400">
              {pendingIncoming}
            </span>
          )}
        </button>
        ...
        {items.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-white/[0.06] py-14 text-center">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-white/[0.03]">
              <svg className="h-6 w-6 text-foreground-muted/20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M7.5 21 3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" />
              </svg>
            </div>
            <p className="mt-3 text-sm text-foreground-muted/50">
              {activeTab === 'incoming' ? t('admin.transfers.noIncoming') : t('admin.transfers.noOutgoing')}
            </p>
          </div>
        )}
      </div>
