...
function GlassSelect({
  ...
  return (
    <select
      ...
      className="w-full rounded-lg border border-border bg-background-secondary px-3 py-2 text-sm text-foreground outline-none transition-colors focus:border-accent/40"
    >
    ...
    <button
      className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-2 text-sm transition-all ${
        hasValue
          ...
          : 'border-border bg-transparent text-foreground-muted hover:bg-background-secondary hover:text-foreground'
      }`}
    >
      {icon}
...
export function FilterPanel({ clubs }: FilterPanelProps) {
  ...
  const statsValue =
    statFilterCount > 0
      ? t('players.activeFilters').replace('{count}', String(statFilterCount))
      : ''
  ...
  return (
    <div className="relative z-10 rounded-2xl border border-border bg-card p-5 shadow-sm space-y-4">
      {/* Layer 1: Search bar */}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          <div className="min-w-[220px] space-y-2">
            <input
              ...
              className="w-full rounded-lg border border-border bg-background-secondary px-3 py-1.5 text-sm text-foreground placeholder-foreground-muted/50 outline-none transition-colors focus:border-accent/40"
            />
            <div className="flex flex-wrap gap-2">
              {clubs
                .filter((c) => {
              ...
              {clubs.filter((c) => {
                ...
                <span className="text-xs text-foreground-muted/50 py-1">
                  {t('players.noResults')}
                </span>
              )}
            </div>
        ...
        >
          <div className="space-y-3 min-w-[240px]">
            <div className="text-[11px] uppercase tracking-wider text-foreground-muted/60 font-medium">
              {t('players.filterAge')}
            </div>
        ...
        >
          <div className="space-y-3 min-w-[200px]">
            ...
            <GlassSelect
              ...
              options={STAT_FILTER_OPTIONS.assists.map((v) => ({
                value: String(v),
                label: `${v}+`,
              }))}
            />
            ...
            <GlassSelect
              ...
              options={STAT_FILTER_OPTIONS.matches.map((v) => ({
                value: String(v),
                label: `${v}+`,
              }))}
            />
            <div className="text-[11px] uppercase tracking-wider text-foreground-muted/60 font-medium">
              {t('players.passAccuracyMin')}
            </div>
            <GlassSelect
              value={passAccMin}
              onChange={(v) => updateParam('pass_acc_min', v)}
              placeholder={t('players.passAccuracyMin')}
              options={STAT_FILTER_OPTIONS.passAccuracy.map((v) => ({
                value: String(v),
                label: `${v}%+`,
              }))}
            />
      ...
      {hasFilters && (
        <div className="flex flex-wrap items-center gap-2 border-t border-border pt-3">
          ...
          {activePositions.map((pos) => (
            <span
              key={pos}
              className="inline-flex items-center gap-1 rounded-full bg-background-secondary px-2.5 py-1 text-[11px] font-medium text-foreground-muted"
            >
              {t(`positions.${pos}`)}
              <button
                onClick={() => toggleMultiParam('position', pos, activePositions)}
                className="ml-0.5 text-foreground-muted/50 hover:text-foreground transition-colors"
              >
                &times;
              </button>
            </span>
          ...
          {activeClubs.map((clubId) => {
            ...
            return (
              ...
              >
                {lang === 'ka' ? c.name_ka : c.name}
                <button
                  onClick={() => toggleMultiParam('club', clubId, activeClubs)}
                  className="ml-0.5 text-foreground-muted/50 hover:text-foreground transition-colors"
                >
                  &times;
                </button>
          ...
          {/* Age tag */}
          {(ageMin || ageMax) && (
            <span className="inline-flex items-center gap-1 rounded-full bg-background-secondary px-2.5 py-1 text-[11px] font-medium text-foreground-muted">
