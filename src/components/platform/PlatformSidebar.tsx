  ...
  return (
    <>
      ...
      <nav className="flex gap-1 overflow-x-auto border-b border-border pb-px md:hidden">
        {links.map((link) => {
          const isActive =
            link.href === '/platform' ? pathname === '/platform' : pathname.startsWith(link.href)
          return (
            <Link
              key={link.href}
              href={link.href}
