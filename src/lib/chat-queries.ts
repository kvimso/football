...
export async function fetchConversations(
  ...
  const conversations: ConversationItem[] = data.map((row) => {
    const scoutDisplayName =
      row.scout_full_name || (row.scout_email ? row.scout_email.split('@')[0] : '')

    const otherParty =
      role === 'scout'
        ? {
            id: row.club_id,
            full_name: row.club_name ?? '',
            organization: null,
            role: 'academy_admin' as const,
          }
        : {
            id: row.scout_id,
            full_name: scoutDisplayName,
            organization: row.scout_organization ?? null,
            role: 'scout' as const,
          }

    const lastMessage =
      row.last_message_content !== null || row.last_message_type !== null
        ? {
...
export async function fetchConversationById(
  ...
  const { safeParse } = await import('zod').then((m) => ({
    safeParse: m.z.string().uuid().safeParse,
  }))
  ...
  const scoutName =
    scout?.full_name ||
    ((scout as Record<string, unknown>)?.email
      ? String((scout as Record<string, unknown>).email).split('@')[0]
      : '')
  ...
  const otherParty =
    role === 'scout'
      ...
      : {
          id: scout?.id ?? '',
          full_name: scoutName,
          organization: scout?.organization ?? null,
  ...
  return {
    ...
    club: club
      ? { id: club.id, name: club.name, name_ka: club.name_ka, logo_url: club.logo_url }
      : { id: '', name: '', name_ka: null, logo_url: null },
    other_party: otherParty,
...
export async function fetchInitialMessages(
  ...
  const normalized: MessageWithSender[] = enriched.reverse().map((msg) => {
    ...
    const refPlayer = Array.isArray(msg.referenced_player)
      ? msg.referenced_player[0]
      : msg.referenced_player
    let playerData = null
    if (refPlayer) {
    ...
    return {
      ...
      sender: sender
        ? { id: sender.id, full_name: sender.full_name, role: sender.role as UserRole | null }
        : null,
