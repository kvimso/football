...
export default async function PlatformDashboardPage() {
  ...
  const stats = [
    {
      label: t('platform.stats.totalPlayers'),
      value: playersResult.count ?? 0,
      href: '/platform/players',
    },
    {
      label: t('platform.stats.totalClubs'),
      value: clubsResult.count ?? 0,
      href: '/platform/clubs',
    },
    {
      label: t('platform.stats.totalScouts'),
      value: scoutsResult.count ?? 0,
      href: '/platform/scouts',
    },
    { label: t('platform.stats.totalMatches'), value: matchesResult.count ?? 0, href: '#' },
    {
      label: t('platform.stats.pendingRequests'),
      value: pendingRequestsResult.count ?? 0,
      href: '/platform/requests',
    },
    {
      label: t('platform.stats.pendingTransfers'),
      value: pendingTransfersResult.count ?? 0,
      href: '/platform/transfers',
    },
    {
      label: t('platform.stats.freeAgents'),
      value: freeAgentsResult.count ?? 0,
      href: '/platform/players?status=free_agent',
    },
  ]

  // Club breakdown
  const { data: clubs } = await admin.from('clubs').select('id, name, name_ka, city').order('name')
    ...
    <div>
      ...
      <div className="mt-8">
        <h2 className="text-lg font-semibold text-foreground">
          {t('platform.dashboard.clubBreakdown')}
        </h2>
          ...
          {(clubs ?? []).map((club) => (
            ...
            >
              <div>
                ...
                {club.city && <p className="text-xs text-foreground-muted">{club.city}</p>}
              </div>
      ...
      <div className="mt-8">
        <h2 className="text-lg font-semibold text-foreground">
          {t('platform.dashboard.recentActivity')}
        </h2>
        {activity.length > 0 ? (
