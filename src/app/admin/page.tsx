...
export default async function AdminDashboardPage() {
  ...
  const viewsAllTime = clubViewCounts.reduce((sum: number, v: { total_views: number }) => sum + Number(v.total_views), 0)
  const viewsThisWeek = clubViewCounts.reduce((sum: number, v: { weekly_views: number }) => sum + Number(v.weekly_views), 0)
  const viewsLastWeek = clubViewCounts.reduce((sum: number, v: { prev_week_views: number }) => sum + Number(v.prev_week_views), 0)
  ...
  const perPlayerViews = clubViewCounts
    .map((v: { player_id: string; total_views: number; weekly_views: number; prev_week_views: number }) => {
  ...
  const stats = [
    ...
    { label: t('admin.stats.scoutSaves'), value: watchlistResult.count ?? 0, href: '#', icon: 'M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.562.562 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.562.562 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z' },
