...
export default async function AdminDashboardPage() {
  ...
  const [
    ...
    viewCountsResult,
    unreadResult,
    demandThisMonthResult,
    demandLastMonthResult,
  ] = await Promise.all([
    playerIds.length > 0
      ? supabase
  ...
  if (requestsResult.error)
    console.error('Failed to fetch request count:', requestsResult.error.message)
  if (watchlistResult.error)
    console.error('Failed to fetch watchlist count:', watchlistResult.error.message)
  if (recentRequestsResult.error)
    console.error('Failed to fetch recent requests:', recentRequestsResult.error.message)
  if (pageViewsResult.error)
    console.error('Failed to fetch page views count:', pageViewsResult.error.message)
  if (scoutActivityResult.error)
    console.error('Failed to fetch scout activity:', scoutActivityResult.error.message)
  if (viewCountsResult.error)
    console.error('Failed to fetch view counts:', viewCountsResult.error.message)
  if (unreadResult.error) console.error('Failed to fetch unread count:', unreadResult.error.message)
  if (demandThisMonthResult.error)
    console.error('Failed to fetch scout demand:', demandThisMonthResult.error.message)
  if (demandLastMonthResult.error)
    console.error('Failed to fetch last month demand:', demandLastMonthResult.error.message)

  // Build per-player view breakdown from RPC data
  const clubViewCounts = viewCountsResult.data ?? []
  const playerNameMap = new Map(
    (clubPlayers ?? []).map((p) => [p.id, { name: p.name, name_ka: p.name_ka }])
  )

  const viewsAllTime = clubViewCounts.reduce(
    (sum: number, v: { total_views: number }) => sum + Number(v.total_views),
    0
  )
  const viewsThisWeek = clubViewCounts.reduce(
    (sum: number, v: { weekly_views: number }) => sum + Number(v.weekly_views),
    0
  )
  const viewsLastWeek = clubViewCounts.reduce(
    (sum: number, v: { prev_week_views: number }) => sum + Number(v.prev_week_views),
    0
  )
  ...
  const stats = [
    {
      label: t('admin.stats.totalPlayers'),
      value: playerCount,
      href: '/admin/players',
      icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z',
    },
    {
      label: t('admin.stats.unreadMessages'),
      value: unreadCount,
      href: '/admin/messages',
      icon: 'M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z',
    },
    {
      label: t('admin.stats.scoutSaves'),
      value: watchlistResult.count ?? 0,
      href: '#',
    ...
    },
    {
      ...
      subtitle: mostViewedPlayer
        ? `${t('admin.stats.mostViewed')}: ${mostViewedPlayer.name}`
        : undefined,
    },
    {
      label: t('admin.stats.viewsAllTime'),
      value: viewsAllTime,
      href: '#',
  ...
  const scoutActivity = ('data' in scoutActivityResult ? (scoutActivityResult.data ?? []) : [])
    .map((v) => ({
      ...v,
  ...
  return (
    <div className="space-y-6">
      ...
      <DashboardStatCards stats={stats} trendLabel={t('admin.stats.viewsTrend')} />

      <div className="grid gap-4 lg:grid-cols-2">
        <DashboardQuickActions
          labels={{
            quickActions: t('admin.common.quickActions'),
            addPlayer: t('admin.players.addPlayer'),
            messages: t('chat.messages'),
            players: t('admin.nav.players'),
            transfers: t('admin.nav.transfers'),
          }}
          unreadCount={unreadCount}
        />

        <DashboardPlayerViews
          title={t('admin.stats.playerViewBreakdown')}
          noActivityLabel={t('admin.stats.noActivity')}
      ...
      <DashboardScoutActivity
        ...
        noActivityLabel={t('admin.stats.noActivity')}
        viewedLabel={t('admin.stats.scoutViewed')}
        unknownLabel={t('matches.unknown')}
